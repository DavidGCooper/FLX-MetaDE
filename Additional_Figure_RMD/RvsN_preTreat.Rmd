...
Results of R vs N for pretreat samples. LCL data came from human patients with known response. Cortex data came from mouse models -- each strain was measured and summarized for response, but the ones included in this comparison were not actually treated (their response is inferred based on strain).
...
resPlot$p.adj <- ifelse(resPlot$padj < 0.0001, yes = "< 0.0001",
                        no = ifelse(resPlot$padj < 0.001, yes = "< 0.001",
                        no = ifelse(resPlot$padj < 0.01, yes = "< 0.01", no = "< 0.05")))

ggplot(resPlot, aes(x = DS, y = pathway, color = NES, size = p.adj)) +
  geom_point() +
  scale_size_manual(values = c(6, 5, 3.75, 2.5)) +
  scale_color_gradientn(limits = c(-2.7, 2.7),
